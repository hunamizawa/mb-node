...
import Module = require('./mbc-m');
import { get_mt } from './mbc';
...
interface ParsedAppArgs extends PrintOptions {
    ipaddr: string,
...
/**
 * Convert options depends on mb library
 */
interface MbdConvertOptions {
    brightness: number,
    saturation: number,
    density: number,
    cal: {
    ...
    /** Printing direction, 0 = left to right, 1 = right to left */
    invert: 0 | 1,
    /** Color order of printer head, 0 = CMY, 1 = CYM */
    c_order: 0 | 1,
    /** Printer head width, 0 = 4.4, 1 = 4.9 */
    c_width: 0 | 1,
    /** Horizontial DPI, 1 = 300, 2 = 600, 4 = 1200 */
    dpi_step: 1 | 2 | 4,
}
 ...
 */
function ensureBetween<T>(a: T, name: keyof T, min: number, max: number, _default: number = null) {
    const v = +a[name];
 ...
 */
function ensureAny<T, U>(a: T, name: keyof T, possibleValues: readonly U[], _default: U = null, preCheck?: (v: any) => U) {
...
function raiseUnknownValue<T>(obj: T, key: keyof T): never {
    throw new ArgsParsingError(`Unknown value '${obj[key]}' for '${key}'`);
}

function convertArgsToOptions(options: PrintOptions): MbdConvertOptions {
...
function convertOptionsToArgs(options: MbdConvertOptions): PrintOptions {
    return {
        brightness: options.brightness,
        saturation: options.saturation,
        density: options.density + 1,
 ...
 * @param options 
 */
async function convert(input: Buffer, options: MbdConvertOptions) {
    ...
    const mt = get_mt({ c: 0xff, m: 0xff, y: 0xff });
...
function parseArgs(): ParsedAppArgs {
...
`;
    ...
    const verbose: typeof console.log = ((v: boolean) => {
        if (v)
            return console.log;
        return () => { };
    })(args['--verbose']);
    ...
    try {
        ...
        const c_order = ensureAny(args, '--c-order', ['cmy', 'cym'] as const, 'cmy', v => v.toLowerCase());
        const c_width = ensureAny(args, '--c-width', [4.4, 4.9] as const, 4.4);
        const dpi = ensureAny(args, '--c-width', [1200, 600, 300] as const, 600);
...
function copyToEnd(src: stream.Readable, dest: stream.Writable) {
 ...
 * @param options 
 */
function expressInit(options: ParsedAppArgs) {
    ...
    function resSetting(res: express.Response<any>) {
        res.json(options);
    }
    ...
    app.post('/setting', bodyParser.json(), (req, res, next) => {
        try {
            ...
            if (req.body.rtl)
                options.rtl = ensureAny(req.body, 'rtl', [true, false] as const);
            if (req.body.c_order)
                options.c_order = ensureAny(req.body, 'c_order', ['cmy', 'cym'] as const);
            if (req.body.c_width)
                options.c_width = ensureAny(req.body, 'c_width', [4.4, 4.9] as const);
            if (req.body.dpi)
                options.dpi = ensureAny(req.body, 'dpi', [300, 600, 1200] as const);
 ...
 * @param options 
 */
async function doJob(options: ParsedAppArgs) {
    ...
    let res = await fetch(`http://${options.ipaddr}/cgi-bin/cmd?cmd=rm_upload`, {
        method: "GET",
        timeout: 3000,
    });
    ...
    for (let i = 0; i < mbds.length; i++) {
        ...
        res = await fetch(`http://${options.ipaddr}/cgi-bin/upload`, {
            method: "POST",
            body: form,
            timeout: 3000,
        });
